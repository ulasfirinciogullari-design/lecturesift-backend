"""Version 5 PostgreSQL contracts including workspace and admin grants.

Historical v2 verification remains in verify_schema_transition.py. This version
admits only the two core and twelve product/workspace additive tables, requires explicit
...
WORKSPACE_TABLES = frozenset(['admin_ad_free_grants_v1', 'admin_order_archives_v1', 'workspace_folders_v1', 'workspace_items_v1'])
PRODUCT_TABLES = PRODUCT_TABLES | WORKSPACE_TABLES
...
def _target_contracts(contract_path: Path) -> dict[str, frozenset[str]]:
    ...
    workspace = _contract(contract_path.with_name("schema_contract_workspace_v1.txt"), prefixes=tuple(prefix for prefix in TARGET_PREFIXES if any(table in prefix for table in WORKSPACE_TABLES)), label="workspace/admin")
    return {
        **{table: _table_objects(product, table) for table in PRODUCT_TABLES - WORKSPACE_TABLES},
        **{table: _table_objects(workspace, table) for table in WORKSPACE_TABLES},
