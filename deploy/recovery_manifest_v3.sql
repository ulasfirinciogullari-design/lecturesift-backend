-- Recovery v3: strict complete product ledger inventory.
\set ON_ERROR_STOP on
\set QUIET on
\pset tuples_only on
\pset format unaligned

-- Version 4: assistant credits and recurring referrals; historical v3 is immutable.
...
\set LECTURESIFT_ALLOW_LEGACY_PRODUCT_TABLES off
...
WHERE a.table_name IS NULL
  AND NOT (e.table_name IN ('assistant_credit_grants_v1','assistant_credit_requests_v1','assistant_daily_budget_v1','billing_referral_codes','billing_referral_coupons','billing_referral_renewal_rewards','billing_referral_reward_preferences','billing_referral_rewards') AND :'LECTURESIFT_ALLOW_LEGACY_PRODUCT_TABLES'::boolean)
  AND NOT (
...
WITH checks(table_name, predicate) AS (VALUES
  ('assistant_credit_grants_v1', 'remaining < 0 OR credits < 0 OR remaining > credits OR expires_at <= created_at OR kind NOT IN (''welcome'',''subscription'',''topup'') OR NOT EXISTS (SELECT 1 FROM billing_users u WHERE u.id=t.user_id)'),
...
FROM expected WHERE to_regclass('public.' || table_name) IS NULL AND :'LECTURESIFT_ALLOW_LEGACY_PRODUCT_TABLES'::boolean ORDER BY table_name;
...
table_diffs AS (
  ...
  WHERE actual.table_name IS NULL
    AND NOT (expected.table_name IN ('assistant_credit_grants_v1','assistant_credit_requests_v1','assistant_daily_budget_v1','billing_referral_codes','billing_referral_coupons','billing_referral_renewal_rewards','billing_referral_reward_preferences','billing_referral_rewards') AND :'LECTURESIFT_ALLOW_LEGACY_PRODUCT_TABLES'::boolean)
    AND NOT (
    ...
    END + CASE
      WHEN to_regclass('public.billing_purchase_terms') IS NULL
       AND :'LECTURESIFT_ALLOW_LEGACY_PURCHASE_TERMS'::boolean
        THEN 1
      ELSE 0
    END + (SELECT count(*) FROM expected WHERE table_name IN ('assistant_credit_grants_v1','assistant_credit_requests_v1','assistant_daily_budget_v1','billing_referral_codes','billing_referral_coupons','billing_referral_renewal_rewards','billing_referral_reward_preferences','billing_referral_rewards') AND to_regclass('public.' || table_name) IS NULL AND :'LECTURESIFT_ALLOW_LEGACY_PRODUCT_TABLES'::boolean) schema_compat_count,
    (
