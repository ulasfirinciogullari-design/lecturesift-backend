...
def test_verification_code_email_uses_mobile_safe_fixed_layout(monkeypatch):
    _enable_verification(monkeypatch)

    subject, text, body = email_auth._email_copy("123456", "tr")

    assert subject == "LectureSift e-posta doğrulama kodun"
    assert "123456" in text
    assert "table-layout:fixed" in body
    assert "letter-spacing:10px" not in body
