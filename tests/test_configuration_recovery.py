...
def test_configuration_snapshot_uses_only_exact_allowlists():
    ...
    assert snapshot.IDENTITY_ALLOWLIST == (
        ...
        "deploy/record_payment_provider_baseline.py",
        "deploy/source_postgres_transport.py",
        "deploy/source_redis_guard.py",
        "deploy/render_worker_stop_evidence.py",
        ...
        "deploy/promote_rehearsed_release.sh",
        "deploy/install_systemd_units.sh",
        "deploy/validate_ingress_health.py",
        "deploy/verify_production_ingress.sh",
        "deploy/handoff_production_ingress.sh",
        "deploy/reconcile_production_ingress.sh",
        "deploy/verify_instagram_publishers_stopped.py",
        "deploy/verify_instagram_publishers_stopped.sh",
        ...
        "deploy/lecturesift-ingress.service",
        "deploy/lecturesift-ingress-selector.service",
        "deploy/lecturesift-caddy-staging.service",
...
def test_configuration_snapshot_covers_the_steady_state_startup_chain_only():
    ...
    steady_state_dependencies = {
        ...
        "deploy/target_redis_manifest.sh",
        "deploy/redis_logical_manifest.py",
        # Public ingress ownership and its fail-closed handoff gate.
        "deploy/Caddyfile.staging",
        "deploy/validate_ingress_health.py",
        "deploy/verify_production_ingress.sh",
        "deploy/handoff_production_ingress.sh",
