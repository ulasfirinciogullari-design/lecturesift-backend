...
AdSense Management API v2 can read site approval state, Auto ads state, alerts and policy issues with `https://www.googleapis.com/auth/adsense.readonly`. It needs a Google OAuth client and consent from the account that owns `lecturesift.com`; a public publisher ID is not authentication. AdSense does not support service accounts. The source integration, opt-in environment gates, loopback OAuth helper and admin-only cached status reader are prepared and contact only Google's fixed OAuth and AdSense API hosts. That source readiness is not a live connection: the helper leaves the runtime flag off, no production OAuth credentials were found or installed during this check, and no provider approval was verified. The reader never activates ads or returns tokens, raw alert HTML, policy URLs or Google resource identifiers. Use the installed-application flow and private credential storage, never a password, browser-cookie copy or secret in chat. [AdSense authorization](https://developers.google.com/adsense/management/direct_requests), [site state reference](https://developers.google.com/adsense/management/reference/rest/v2/accounts.sites)
