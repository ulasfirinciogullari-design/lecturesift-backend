...
<header class="guide-header"><div class="guide-header-inner"><a class="brand guide-brand" href="${prefix}/" aria-label="LectureSift ${tr ? "ana sayfa" : "home"}">${brand}</a><nav aria-label="${tr ? "Ana menü" : "Main navigation"}"><a href="${prefix}/study-guides"${library ? ' aria-current="page"' : ""}>${tr ? "Çalışma rehberleri" : "Study guides"}</a><a href="${prefix}/about-study-guides"${page.slug === "about-study-guides" ? ' aria-current="page"' : ""}>${tr ? "Rehberler hakkında" : "About the guides"}</a><a class="guide-language" href="${alternate}" hreflang="${tr ? "en" : "tr"}" lang="${tr ? "en" : "tr"}">${tr ? "English" : "Türkçe"}</a></nav><a class="guide-button guide-button-primary guide-header-cta" href="${prefix}/workspace.html">${tr ? "Çalışma alanı" : "Workspace"} <span aria-hidden="true">↗</span></a></div></header>
